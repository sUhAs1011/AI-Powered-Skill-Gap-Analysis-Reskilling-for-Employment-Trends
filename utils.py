...
SKILL_NORMALIZATION_MAP = {
    ...
    "chemical mine safety": "chemical mine security",
    "biological mine safety": "biological mine security",
    "radiological mine safety": "radiological mine security",
    "nuclear biological and chemical mine safety": "nuclear biological and chemical mine security",
    "nbc mine safety": "nuclear biological and chemical mine security",
    # Add more as needed
}
...
def normalize_skill(skill_text, normalization_map):
    ...
    norm = ' '.join(tokens)
    # Synonym map
    return normalization_map.get(norm, norm)
...
def fuzzy_skill_overlap(job_skills, course_skills, threshold=0.85):
    matched = set()
    for job_skill in job_skills:
        job_doc = nlp(job_skill)
        for course_skill in course_skills:
            course_doc = nlp(course_skill)
            if job_doc.similarity(course_doc) >= threshold:
                matched.add(job_skill)
                break
    return matched

def normalize_skill_name(skill):
    """Normalize a single skill string using the normalization map and NLP logic."""
    return normalize_skill_list([skill], SKILL_NORMALIZATION_MAP)[0] if skill else ""

def extract_skills_from_text(text, normalization_map=SKILL_NORMALIZATION_MAP):
    """
    Extracts candidate skills from free text using spaCy noun chunks and entities.
    Returns a list of normalized, deduplicated skills.
    """
    if not isinstance(text, str) or not text.strip():
        return []
    doc = nlp(text)
    # Extract noun chunks and named entities as candidate skills
    candidates = set(chunk.text.lower().strip() for chunk in doc.noun_chunks)
    candidates.update(ent.text.lower().strip() for ent in doc.ents)
    # Remove stopwords, punctuation, and normalize
    skills = []
    for cand in candidates:
        cand = cand.translate(str.maketrans('', '', string.punctuation))
        if cand and cand not in stop_words and len(cand) > 2:
            norm = normalize_skill(cand, normalization_map)
            if norm:
                skills.append(norm)
